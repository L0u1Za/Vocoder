    ...
    def _log_predictions(
        ...
        for text_cur, audio_cur, audio_pred_cur, audio_path_cur in tuples[:examples_to_log]:
            ...
            rows[Path(audio_path_cur).name] = {
                "text": text_cur,
                "audio": self.writer.wandb.Audio(audio_cur.detach().numpy().cpu(), sample_rate=self.config["preprocessing"]["sr"]),
                "predicted_audio": self.writer.wandb.Audio(audio_pred_cur.detach().numpy().cpu(), sample_rate=self.config["preprocessing"]["sr"])
            }
